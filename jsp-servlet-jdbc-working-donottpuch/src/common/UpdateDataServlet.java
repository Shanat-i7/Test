...
public class UpdateDataServlet extends HttpServlet {
	...
	protected void doPost(HttpServletRequest uuu, HttpServletResponse response) throws ServletException, IOException {
		...
		Database database878 = new Database();
		database878.UpdaterData(employee);
	
		
		List<Employee> empDataSet = database878.getDataCust();
		
		uuu.setAttribute("empData", empDataSet);
		
				
		uuu.getRequestDispatcher("/jiji.jsp").forward(uuu, response);
